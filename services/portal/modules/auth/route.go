...
func RegisterRoutes(e *echo.Echo, repo repository, keycloak config.Keycloak, client *http.Client, privateKey *rsa.PrivateKey, keyfunc jwt.Keyfunc) {
	s := newService(repo, keycloak, client, privateKey, keyfunc)
	h := newHandler(s)
