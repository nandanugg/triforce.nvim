...
func Test_handler_listAdminPPNPN(t *testing.T) {
	t.Parallel()

	dbData := `
		INSERT INTO ref_unit_kerja (id, diatasan_id, nama_unor, nama_jabatan, pemimpin_pns_id, deleted_at) VALUES
		...
		INSERT INTO pegawai_ppnpn (id, nama, nik, unit_kerja_id, jabatan_id, foto, deleted_at) VALUES
			(2001, 'Fajar Hidayat', '317xxxxxxxx', 'unor-1', 'JBT-001', 'fajar.png', NULL),
			(2002, 'Nur Aisyah', '317yyyyyyyy', 'unor-2', NULL, NULL, NULL),
			(2003, 'Hilmi Kurniawan', '317zzzzzzzz', 'unor-1', 'JBT-001', NULL, NOW());  -- deleted
	`
	...
	tests := []struct {
		...
		},
		{
			name:             "keyword nama",
			requestHeader:    http.Header{"Authorization": authHeader},
			requestQuery:     url.Values{"keyword": []string{"Fajar"}},
			wantResponseCode: http.StatusOK,
			wantResponseBody: `
			{
				"data": [{
		...
		},
		{
			name:             "keyword nik",
			requestHeader:    http.Header{"Authorization": authHeader},
			requestQuery:     url.Values{"keyword": []string{"317y"}},
			wantResponseCode: http.StatusOK,
			wantResponseBody: `
			{
				"data": [{
		...
		{
			name:             "filter unit kerja",
			requestHeader:    http.Header{"Authorization": authHeader},
			requestQuery:     url.Values{"unor_id": []string{"unor-1"}},
			wantResponseCode: http.StatusOK,
			wantResponseBody: `
			{
				"data": [{
		...
		{
			name:             "invalid token",
			requestHeader:    http.Header{"Authorization": []string{"Bearer something"}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.requestQuery.Add("tipe", tipePegawaiPPNPN)
...
func Test_handler_getAdmin(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodGet, "/v1/admin/pegawai/"+tt.nip, nil)
			req.Header = tt.requestHeader
			rec := httptest.NewRecorder()
