...
func RegisterRoutes(e *echo.Echo, repo repository, db *pgxpool.Pool, mwAuth api.AuthMiddlewareFunc) {
	...
	e.Add(http.MethodGet, "/v1/admin/pegawai/pppk", h.listAdminPPPK, mwAuth(api.Kode_Pegawai_Read))
	e.Add(http.MethodGet, "/v1/admin/pegawai/nonaktif", h.listAdminNonAktif, mwAuth(api.Kode_Pegawai_Read))
