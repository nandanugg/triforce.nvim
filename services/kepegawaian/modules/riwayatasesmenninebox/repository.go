...
func (r *repository) list(ctx context.Context, userID int64, limit, offset uint) ([]riwayatAsesmenNineBox, error) {
	...
	result := []riwayatAsesmenNineBox{}
	for rows.Next() {
		var row riwayatAsesmenNineBox
		err := rows.Scan(
			&row.ID,
			&row.Tahun,
			&row.Kesimpulan,
