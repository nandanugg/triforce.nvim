...
func (h *handler) adminGet(c echo.Context) error {
	...
	if err != nil {
		slog.ErrorContext(ctx, "Error getting jenis kenaikan pangkat.", "error", err)
		return echo.NewHTTPError(http.StatusInternalServerError)
	}
...
func (h *handler) adminCreate(c echo.Context) error {
	...
	if err != nil {
		slog.ErrorContext(ctx, "Error creating jenis kenaikan pangkat.", "error", err)
		return echo.NewHTTPError(http.StatusInternalServerError)
	}
...
func (h *handler) adminDelete(c echo.Context) error {
	...
	if err != nil {
		slog.ErrorContext(ctx, "Error deleting jenis kenaikan pangkat.", "error", err)
		return echo.NewHTTPError(http.StatusInternalServerError)
	}
