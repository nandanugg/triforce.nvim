...
func (h *handler) list(c echo.Context) error {
	...
	if err != nil {
		slog.ErrorContext(ctx, "Error getting list riwayat pelatihan teknis.", "error", err)
...
func (h *handler) getBerkas(c echo.Context) error {
	...
	if err != nil {
		slog.ErrorContext(ctx, "Error getting berkas riwayat pelatihan teknis.", "error", err)
		return echo.NewHTTPError(http.StatusInternalServerError)
	}

	if blob == nil {
		return echo.NewHTTPError(http.StatusNotFound, "berkas riwayat pelatihan teknis tidak ditemukan")
	}
